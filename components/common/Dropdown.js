...
export default function Dropdown({ name, menuItems }) {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      ...
      >
        <Menu.Items className="absolute right-0 z-10 mt-1 w-full origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
            {menuItems.map((item) => (
              <Menu.Item>
                {({ active }) => (
